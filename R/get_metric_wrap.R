...
getOutput_ensemble_cv <- function(res,true,trueMet,cal_mean = FALSE){
  ...
  df1 <- suppressWarnings(df %>% group_by(CellType,Method) %>%
    mutate(RMSE= sqrt(mean((p_hat-p_true)^2))%>% round(.,2),
           ...
           Pearson = cor(p_hat,p_true,use = 'complete',method = "spearman")%>% round(.,2),
    ...
    group_by(Method) %>% mutate(RMSE_Total = sqrt(mean((p_hat-p_true)^2))%>% round(.,2),
                                ...
                                Pearson_Total = cor(p_hat,p_true,use = 'complete',method = "spearman")%>% round(.,2)))

  df2 <- df1 %>%  group_by(Method) %>%   mutate(RMSE_median = median(unique(RMSE)) %>% round(.,2),
                                               Pearson_median=median(unique(Pearson),na.rm = T) %>% round(.,2),
                                               Pearson_sd=sd(unique(Pearson),na.rm = T) %>% round(.,2),
                                               RMSE_mean = mean(RMSE) %>% round(.,2),
                                               Pearson_mean=mean(Pearson,na.rm = T) %>% round(.,2),
                                               Pearson_max = max(Pearson,na.rm =T)%>% round(.,2),
                                               Pearson_min = min(Pearson,na.rm =T)%>% round(.,2),
                                               Pearson_max_celltype = ifelse(Pearson == Pearson_max,as.character(CellType),NA),
                                               Pearson_min_celltype = ifelse(Pearson == Pearson_min,as.character(CellType),NA))
