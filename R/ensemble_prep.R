...
########### get_input_ensemble ###########
get_input_ensemble <- function(count_bulk, ref_matrix, meta_bulk, meta_ref, true_frac = NULL,params,ncv_input = 2,myseed = 2020,clustering = TRUE) {
    ...
    if(clustering){
        tmp <- kmeans(log2(tmp_count_bulk+1),2)
        cv_fold <- lapply(1:2, function(i) which(tmp$cluster == i))
    }else{
        cv_fold <- createFolds(1:nrow(count_bulk),k=ncv_input)
    }

    # Perform cross validation
    if(ncv_input > 1){
        data_set <- lapply(cv_fold, cv_set,count_bulk = count_bulk,ref_matrix = ref_matrix,meta_bulk = meta_bulk,meta_ref = meta_ref,params = params,
                           true_frac = true_frac)
    }else{
...
get_set_basis <- function(count_bulk = count_bulk,ref_matrix = ref_matrix,meta_bulk = meta_bulk,meta_ref = meta_ref,params = params,true_frac = true_frac){
    ...
    if(data_type == "singlecell-rna"){
        ...
        if(is.null(true_frac)){
            true_frac <- matrix(NA, nrow = nrow(meta_bulk), ncol = K)
        }  else{
