...
#' @param feature.select Logical. Perform feature selection by R2 or not. Default : FALSE.
#' @param clustering Logical. Apply Kmeans custering on bulk RNA seq data. Default : TRUE.
...
#'
#'
gen_all_res_list = function(count_bulk,meta_bulk = NULL,ref_list,customed_markers = NULL,markers_range = NULL,true_frac = NULL,params = NULL,ncv_input =2,
                            outpath,parallel_comp = FALSE,ncore,os = "win",myseed = 2020,feature.select = FALSE,clustering = TRUE,rm.duplicated =FALSE,mrkpen = FALSE,dmeths = NULL){
  ...
  clusterCall(cl, function(x) .libPaths(x), .libPaths())
    res_all = foreach(i = 1:nrow(params),.options.snow = opts, .errorhandling='pass',
                      .packages = c("SCDC","MuSiC","BisqueRNA","TED","TOAST","EPIC", "dtangle","CellMix","nnls","xbioc","Biobase","scran","DeCompress","ICeDT","DeconRNASeq","preprocessCore","glmnet","sva","hspe","RVenn","markerpen","FARDEEP","ComICS","edgeR","BayICE","reticulate")) %dopar% {
      p = params[i,]
      ...
      capture.output({
        ...
        Dataset = get_input_ensemble(count_bulk = count_bulk, ref_matrix = ref_list[[p$data_name]]$ref_matrix, meta_bulk = meta_bulk,
                                     meta_ref = ref_list[[p$data_name]]$meta_ref, true_frac = true_frac,params = p,ncv_input = ncv_input,myseed = myseed,clustering = clustering)
        ensemble_dat = lapply(Dataset,function(x) x[["ensemble"]])
# 
#         if(is.null(dmeths)){# Get deconvolution methods
#         dmeths_ori <- c("dtangle", "hspe","deconf","ssFrobenius","ssKL","DSA","Q Prog","LS Fit","CIBERSORT","logRegression","linearRegression","EPIC","TOASTP","MuSiC","BisqueRNA","GEDIT", "ICeDT","DeconRNASeq", "BayesPrism")
# 
#         if (p$Scale == "log"){
...
#         }}
        ...
        dir.create(paste0(outpath,p$data_name,"/","Fold_",ncv_input,"/"), showWarnings = FALSE, recursive = TRUE)
        saveRDS(list(a = a, p = p,ensemble = ensemble_dat), file = paste0(outpath,p$data_name,"/","Fold_",ncv_input,"/", paste0(params[i, ], collapse = "_"),  ".rds"))
    ...
    for(i in 1:nrow(params)){
      ...
      require(SCDC)
      Dataset = get_input_ensemble(count_bulk = count_bulk, ref_matrix = ref_list[[p$data_name]]$ref_matrix, meta_bulk = meta_bulk,
                                   meta_ref = ref_list[[p$data_name]]$meta_ref, true_frac = true_frac,params = p,ncv_input = ncv_input,clustering = clustering)
      ensemble_dat = NULL
      if(ncv_input >1){
        ensemble_dat = lapply(Dataset,function(x) x[["ensemble"]])
      }
